...
class ConnectionHandler:
    def __init__(
        ...
        self.client_ip_info = {}
        self.prompt = None
        self.assistant_name = None  # 添加助手名称属性
    ...
    def change_system_prompt(self, prompt):
        self.prompt = prompt
        # 从prompt中提取助手名称
        self._extract_assistant_name(prompt)
        # 更新系统prompt至上下文
        self.dialogue.update_system_message(self.prompt)

    def _extract_assistant_name(self, prompt):
        """从系统提示词中提取助手名称"""
        import re
        if not prompt:
        ...
        for pattern in patterns:
            ...
            if match:
                ...
                # 过滤掉一些通用词汇
                if name and name not in ['助手', '机器人', 'AI', '人工智能', '智能助手']:
                    self.assistant_name = name
                    self.logger.bind(tag=TAG).info(f"提取到助手名称: {self.assistant_name}")
                    return
        
        # 如果没有匹配到，设置为None
        self.assistant_name = None
        self.logger.bind(tag=TAG).debug("未能从prompt中提取到助手名称")
